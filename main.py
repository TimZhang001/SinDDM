...
def parse_args():
    ...
    parser.add_argument("--clip_text", help='enter CLIP text.', default='crack in the hazelnut') #'Fire in the Forest')
    
    # # relevant if mode==clip_content
    parser.add_argument("--fill_factor", help='Dictates relative amount of pixels to be changed. Should be between 0 and 1.',  type=float, default=0.2)
    parser.add_argument("--strength",    help='Dictates the relative strength of CLIPs gradients. Should be between 0 and 1.', type=float, default=0.2)
    ...
    # device num
    parser.add_argument("--device_num",        help='use specific cuda device.', default=7, type=int)
...
def main():
    ...
    if args.mode == 'train':
        ...
        #Tim
        #roi = cv2.selectROI(image_to_select) #[50,80,50,50],
        roi = [50,80,50,50]
        roi_perm = [1, 0, 3, 2]
        ...
        ScaleTrainer.image2image(input_folder=i2i_folder, input_file=args.input_image, mask=args.harm_mask, hist_ref_path=hist_ref_path,
                                 ...
                                 device=device, use_hist=use_hist, save_unbatched=save_unbatched, auto_scale=50000, mode=args.mode)
    else:
